...
module.exports = class DeployResources {
	...
	async deployAccount(name, account) {
		...
		if (foundAccount) {
			...
			if (!sameRoles) {
				this.serverless.cli.log(`WARNING!!! Different roles for account ${name}`);
	...
	async deployDatabase(name, database) {
		...
		database.id = db.response.id;
		database.endpoint = db.response.endpoint;
	}
	...
	async deployApiGateway(name, gateway) {
		...
		gateway.id = gw.metadata.apiGatewayId;
	}
