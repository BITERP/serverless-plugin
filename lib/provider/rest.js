...
const ENDPOINTS = {
	...
	"func": "https://serverless-functions.api.cloud.yandex.net"
};
...
module.exports = class REST {
	...
	async removeScalingPolicy(functionId) {
		const post = {
			tag: "$latest"
		};
		return await this.restCall("func", `functions/v1/functions/${functionId}:removeScalingPolicy`, post);
	}

	async setScalingPolicy(functionId, instances) {
		const post = {
		  tag: "$latest",
		  provisionedInstancesCount: `${instances}`,
		  zoneInstancesLimit: "0",
		  zoneRequestsLimit: "0"
		};
		return await this.restCall("func", `functions/v1/functions/${functionId}:setScalingPolicy`, post);
	}
