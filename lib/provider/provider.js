...
class YandexCloudProvider {
    ...
    async getFunctionVersionByTag(func, tag) {
        await this.initConnectionsIfNeeded();
        const result = await this.functions.getVersionByTag({
            functionId: func.id,
            tag
        });
        return result;
    ...
    async getFunctionVersions(functionId){
        await this.initConnectionsIfNeeded();
        const result = await this.functions.listVersions({
    ...
    async removeFunctionTag(functionId, versionId, tag) {
        await this.initConnectionsIfNeeded();
        const operation = await this.functions.removeTag({
    ...
    async createFunction(request) {
        ...
        return await this.updateFunction(request);
    }
